...
public class TypeContent<T, E extends AnnotatedElement> extends TypeData<E> {
    private final Class<T> content;

    public TypeContent(E element, Class<T> content) {
        super(element);
    ...
    public boolean hasScalarContent() {
        return TypeFactory.SCALAR_MAP.containsKey(content);
    }
    ...
    private GraphQLType GraphQLTypeByReferenceType() {
        if (hasScalarContent()) {
            return TypeFactory.SCALAR_MAP.get(content);
        } else {
